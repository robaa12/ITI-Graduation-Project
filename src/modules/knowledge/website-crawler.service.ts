...
const SKIPPED_PATH = /\/(?:account|login|logout|cart|checkout|search|wp-admin)(?:\/|$)/i;
...
export class WebsiteCrawlerService {
  async crawl(rawUrl: string): Promise<WebsiteCrawlResult> {
    ...
    for (let depth = 0; depth <= MAX_DEPTH && pending.length && pages.length < MAX_PAGES; depth += 1) {
      const next: string[] = [];
      const batch = pending.filter((url) => {
        if (visited.has(url) || !this.isAllowed(url, allowedHost, robots)) return false;
        visited.add(url);
        return true;
      }).slice(0, MAX_PAGES - pages.length);
      if (!batch.length) { pending = next; continue; }
      ...
      const crawler = new CheerioCrawler({
        ...
        async requestHandler({ $, request, response }) {
          ...
          const bytes = (response?.body as { length?: number } | undefined)?.length ?? 0;
          if (bytes > MAX_PAGE_BYTES) {
            warnings.push(`${loadedUrl} — page exceeds the 5 MB ingestion limit`);
            return;
          }
          ...
          if (extracted.content.length >= MIN_STATIC_TEXT) {
            ...
            if (remaining > 0) {
              const content = extracted.content.slice(0, Math.min(MAX_PAGE_TEXT, remaining));
              pages.push(makePage(extracted.url, extracted.title, content));
              totalCharacters += content.length;
            }
          } else warnings.push(`${loadedUrl} — no readable static HTML content was available`);
          if (depth < MAX_DEPTH && totalCharacters < MAX_SITE_TEXT) {
            $('a[href]').each((_, element) => {
              const href = $(element).attr('href');
              const candidate = href ? safeCanonicalUrl(href, loadedUrl) : undefined;
              if (candidate && isAllowed(candidate, allowedHost, robots) && !visited.has(candidate)) next.push(candidate);
            });
          }
        },
        failedRequestHandler({ request, error }) {
          warnings.push(`${request.url} — ${error instanceof Error ? error.message : String(error)}`);
        },
      });
      await crawler.run();
    ...
    if (!pages.length) throw new Error(warnings[0] ?? 'No readable public HTML pages were available');
  ...
  private async loadRobots(seed: string) {
    ...
    try {
      const response = await fetch(`${origin}/robots.txt`, { signal: AbortSignal.timeout(8_000) });
      return robotsParser(`${origin}/robots.txt`, response.ok ? await response.text() : '');
    } catch {
  ...
  private isAllowed(rawUrl: string, host: string, robots: ReturnType<typeof robotsParser>) {
...
export function extractWithCheerio($: any, url: string) {
  ...
  const title = normalizeText($('title').first().text() || $('h1').first().text()) || new URL(url).hostname;
  ...
  const content = ['main', 'article', 'body']
    .flatMap((selector) => $(selector).toArray().map((element: unknown) => normalizeText($(element).text())))
    .reduce((longest, candidate) => candidate.length > longest.length ? candidate : longest, '');
  return { url: safeCanonicalUrl(canonical ?? url, url) ?? url, title, content };
}
...
function makePage(url: string, title: string, content: string): CrawledWebsitePage {
  return { url, title, content, contentHash: createHash('sha256').update(content).digest('hex') };
}
